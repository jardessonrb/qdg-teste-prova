...
public class DataInitializerUsers implements CommandLineRunner {
    ...
    public void run(String... args) {
        log.info("Iniciando migração de usuários iniciais.");
        try{
            List<UserCreateRequest> validsUsers = this.generateValidsUsers();
            for(var user : validsUsers){
                if(!userRepository.existsByCpf(user.cpf().replaceAll("\\D", ""))){
                    userService.createUser(user);
                }
            }
            log.info("Finalizada a migração de usuários iniciais com sucesso.");
        } catch (Exception exception){
            log.error("Erro ao processar migração de usuário iniciais.");
        }
    }

    public List<UserCreateRequest> generateValidsUsers(){
